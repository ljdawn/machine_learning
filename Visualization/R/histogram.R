...
p_3 <-p3 + labs(title = "kbed_times_histogram") + geom_histogram(binwidth = 1) + xlab("kbed times(binwidth = One time)") + ylab("time counts")+ scale_x_continuous(limits=c(1, 30))
p_3
length(kb_days[kb_days <= 30])/length(kb_days$days)
summary(kbed_times$times)
length(kbed_times[kbed_times <= 30])/length(kbed_times$times)